...
unsigned char ctrl_device_value(char* parameter)
{
	...
	msg.arg[0] = MachineInfo.ValueStatus;//发酵罐出酒阀状态
	SetTimeStampIntoArrary(msg.arg+1);
	UploadDataByWifi(&msg);

	register_callback_function_into_timer(register_value_into_timer(1000, 0),AutoUpload);//返回出就阀状态
